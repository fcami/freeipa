...
"""This module provides tests for NFS-related features like
   ...
   * better validation of "ipa automountlocation-tofiles" output
...
class TestAutomountLocations(TestInit):

    @classmethod
    def install(cls, mh):
        # cls.fix_resolv_conf(cls.master, cls.master)
    ...
    def test_automountlocationtofiles(self):
        ...
        nfssrv = self.master
        location = "portland"
        automountmap = "auto.test"

        self.master.run_command([
            "ipa", "automountlocation-add", location
        ])
        self.master.run_command([
            "ipa", "automountmap-add", location, automountmap
        ])
        keys = ("test1", "test2")
        for key in keys:
            self.master.run_command([
                "ipa", "automountkey-add", location, automountmap,
                "--key=%s" % key,
                "--info=%s:/export/%s" % (nfssrv.hostname, key)
            ])

        cmd = self.master.run_command([
            "ipa", "automountlocation-tofiles", location
        ])
        assert ("test1" and "test2") in cmd.stdout_text

        for key in keys:
            self.master.run_command([
                "ipa", "automountkey-del", location, automountmap,
                "--key=%s" % key,
                "--info=%s:/export/%s" % nfssrv.hostname, key
            ])
        self.master.run_command([
            "ipa", "automountmap-del", location, automountmap
        ])
        self.master.run_command([
...
class TestNFS(TestInit):
    ...
    def test_krb5_nfsd(self):
        ...
        # NFS share
        # TODO use a better way to associate server-side exports
        #      with client-side mountpoints and options
        basedir = "exports"
        exports = {
            "krbnfs": "*(sec=krb5p,rw)",
            "stdnfs": "*(ro)",
    ...
    def test_krb5_nfs_manual_configuration(self):
        ...
        # nfsclt.run_command([
        #    "mount", "-t", "nfs4", "-o", "ro",
        #    "%s:/exports/stdnfs" % nfssrv.hostname, "/mnt/std", "-v"
        # ])
        nfsclt.run_command([
